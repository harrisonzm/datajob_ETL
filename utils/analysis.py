def salary_by_job_category_analysis():
    ...
    for category in df["job_title_short"].unique():
        ...
        any_salary_count = (
            category_df["salary_year_avg"].notna()
            | category_df["salary_hour_avg"].notna()
        ).sum()
        both_salary_count = (
            category_df["salary_year_avg"].notna()
            & category_df["salary_hour_avg"].notna()
    ...
    print("-" * 100)
    print(
        f"{'Categoría':<25} {'Total':<8} {'Year':<6} {'Hour':<6} {'Any':<6} {'Both':<6} {'None':<6} {'%Year':<7} {'%Hour':<7} {'%Any':<7}"
    )
    print("-" * 100)

    for _, row in analysis_df.iterrows():
        print(
            f"{row['job_category']:<25} "
            f"{row['total_jobs']:<8} "
    ...
    for _, row in top_5_jobs.iterrows():
        print(
            f"• {row['job_category']}: {row['total_jobs']} trabajos "
    ...
    for _, row in top_5_salary.iterrows():
        print(
            f"• {row['job_category']}: {row['any_salary_pct']}% con salario "
...
def payment_analysis():
    ...
    print(
        "sumExceptHourWith: ",
        job_rate[job_rate["salary_rate"] != "hour"]["count"].sum(),
    )
    print(
        "hasPay: ",
        (df["salary_hour_avg"].notna() | df["salary_year_avg"].notna()).sum(),
    )
...
def aggregation_analysis():
    ...
    print(
        f'Porcentaje que contiene "via": {len(filtered_job_via)/total_job_via*100:.2f}%'
    )
    ...
    print(
        f'Porcentaje que NO contiene "via": {len(filtered_no_via)/total_job_via*100:.2f}%'
    )
...
def encoding_errors():
    ...
    if errors_job_title > 0:
        ...
        error_examples_title = df[job_title_clean][job_title_with_errors][
            "job_title"
        ].head(10)
