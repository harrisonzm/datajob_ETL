...
def analizar_dataset(csv_path: str):
    ...
    print(
        f"stg_job_postings                : {resultados['stg_job_postings']:,} registros"
    )
    ...
    print(
        f"stg_job_skills                  : {resultados['stg_job_skills']:,} registros"
    )
    print()
    ...
    df_companies["company_name_clean"] = df_companies["company_name_clean"].str.replace(
        r"[^\w\s+().-]", "", regex=True
    )
    ...
    df_locations["job_location_clean"] = df_locations["job_location_clean"].str.replace(
        r"[^\w\s+().-]", "", regex=True
    )
    df_locations["job_country_clean"] = df_locations["job_country"].str.strip()
    # Eliminar duplicados de la combinación location + country
    unique_locations = df_locations[
        ["job_location_clean", "job_country_clean"]
    ].drop_duplicates()
    resultados["dim_locations"] = len(unique_locations)
    print(
        f"dim_locations                   : {len(unique_locations):,} registros únicos"
    )
    ...
    def clean_via(x):
        ...
        if x_lower.startswith("via "):
            ...
            result = (
                pd.Series([x_str]).str.replace(r"[^\w\s+().-]", "", regex=True).iloc[0]
            )
            result = result.strip()
    ...
    df_vias_clean = df_vias[df_vias["job_via_clean"].notna()][
        ["job_via_clean"]
    ].drop_duplicates()
    ...
    df_schedule["job_schedule_type_clean"] = df_schedule[
        "job_schedule_type"
    ].str.strip()
    # Eliminar duplicados después de limpieza
    ...
    print(
        f"dim_schedule_types              : {unique_schedule_types:,} registros únicos"
    )

    # dim_short_titles (aplicar limpieza)
    df_short_titles = df[df["job_title_short"].notna()].copy()
    df_short_titles["job_title_short_clean"] = df_short_titles[
        "job_title_short"
    ].str.strip()
    # Eliminar duplicados después de limpieza
    df_short_titles_unique = df_short_titles[
        ["job_title_short_clean"]
    ].drop_duplicates()
    ...
    if not df_skills.empty:
        ...
        skills_con_tipo = df_skills[df_skills["type_name"].notna()][
            ["skill_name"]
        ].drop_duplicates()
    ...
    print(
        f"dim_skills                      : {resultados['dim_skills']:,} registros únicos"
    )

    # dim_types
    if not df_skills.empty:
        # Eliminar duplicados de type_name
        types_unicos = df_skills[df_skills["type_name"].notna()][
            ["type_name"]
        ].drop_duplicates()
    ...
    print(
        f"dim_types                       : {resultados['dim_types']:,} registros únicos"
    )
    print()
    ...
    print(
        f"fact_job_posts                  : {resultados['fact_job_posts']:,} registros"
    )

    # skill_types (relación entre skills y types)
    if not df_skills.empty:
        skill_types_unique = df_skills[df_skills["type_name"].notna()][
    ...
    print(
        f"skill_types                     : {resultados['skill_types']:,} registros únicos"
    )
    ...
    if not df_skills.empty:
        ...
        job_skills_unique = job_skills_con_tipo[
            ["job_posting_id", "skill_name", "type_name"]
        ].drop_duplicates()
        resultados["job_skills"] = len(job_skills_unique)
    else:
        resultados["job_skills"] = 0
    print(
        f"job_skills                      : {resultados['job_skills']:,} registros únicos"
    )
    print()
    ...
    print(
        f"Registros con job_skills        : {registros_con_job_skills:,} ({registros_con_job_skills/len(df)*100:.2f}%)"
    )
    print(
        f"Registros con job_type_skills   : {registros_con_type_skills:,} ({registros_con_type_skills/len(df)*100:.2f}%)"
    )

    # Contar elementos totales en arrays
    total_skills_en_arrays = sum(
        len(skills) for skills in df["job_skills_parsed"] if isinstance(skills, list)
    )
    total_skills_en_json = sum(
        sum(
            len(skills_list)
            for skills_list in type_skills.values()
            if isinstance(skills_list, list)
        )
        for type_skills in df["job_type_skills_parsed"]
