...
class TestExtraction:
    ...
    def test_job_posting_schema(self):
        ...
        if expected_columns.issubset(actual_columns):
            logger.info(
                f"✓ Todas las {len(expected_columns)} columnas esperadas están presentes"
            )
    ...
    def test_clean_df_function(self):
        ...
        logger.info(
            f"Valores nulos detectados - job_title: {null_titles}, company_name: {null_companies}"
        )
    ...
    def test_parse_skills_string(self):
        ...
        for input_val, expected in test_cases:
            ...
            logger.debug(
                f"Input: {input_val} -> Output: {result} (Expected: {expected})"
            )
    ...
    def test_data_quality_checks(self):
        ...
        with engine.connect() as conn:
            ...
            skills_percentage = (
                (skills_count / total_count * 100) if total_count > 0 else 0
            )
            logger.info(
                f"Registros con job_skills: {skills_count:,} ({skills_percentage:.1f}%)"
            )
            ...
            type_skills_percentage = (
                (type_skills_count / total_count * 100) if total_count > 0 else 0
            )
            logger.info(
                f"Registros con job_type_skills: {type_skills_count:,} ({type_skills_percentage:.1f}%)"
            )
    ...
    def test_posted_date_range_2023(self):
        ...
        with engine.connect() as conn:
            ...
            if dates_before_2023 == 0:
                ...
                logger.error(
                    f"✗ Se encontraron {dates_before_2023} fechas anteriores a 2023"
                )

            assert (
                dates_before_2023 == 0
            ), f"Se encontraron {dates_before_2023} fechas anteriores a 2023. Todas las fechas deben ser >= {min_date.date()}"
