...
class TestBusinessLogic:
    ...
    def test_data_completeness_end_to_end(self):
        ...
        with engine.connect() as conn:
            ...
            source_count = conn.execute(text("SELECT COUNT(*) FROM job_posting")).scalar()
            staging_count = conn.execute(text(f"SELECT COUNT(*) FROM {dbt_table('stg_job_postings')}")).scalar()
            fact_count = conn.execute(text(f"SELECT COUNT(*) FROM {dbt_table('fact_job_posts')}")).scalar()
            
            # Validar que no perdemos más del 5% de datos en cada etapa
            staging_loss = (source_count - staging_count) / source_count * 100
            fact_loss = (staging_count - fact_count) / staging_count * 100 if staging_count > 0 else 0
            
            assert staging_loss <= 5.0, f"Pérdida excesiva en staging: {staging_loss:.2f}%"
            assert fact_loss <= 5.0, f"Pérdida excesiva en facts: {fact_loss:.2f}%"
            
            print(f"✅ Pipeline completo: {source_count:,} → {staging_count:,} → {fact_count:,}")
    ...
    def test_skills_extraction_accuracy(self):
        ...
        with engine.connect() as conn:
            ...
            if source_jobs_with_skills > 0 and processed_jobs_with_types == 0:
                print("⚠️  PROBLEMA CRÍTICO: Procesamiento de JSON job_type_skills no implementado")
                print("   Esto es esperado ya que simplificamos el modelo para solo procesar job_skills array")
                ...
                assert accuracy >= 80.0, f"Precisión de extracción muy baja: {accuracy:.2f}%"
    ...
    def test_salary_data_preservation(self):
        ...
        with engine.connect() as conn:
            ...
            if source_salary_jobs > 0:
                ...
                assert preservation >= 95.0, f"Pérdida excesiva de datos de salario: {preservation:.2f}%"
                print(f"✅ Preservación salarios: {preservation:.2f}% ({fact_salary_jobs:,}/{source_salary_jobs:,})")
    ...
    def test_company_normalization_quality(self):
        ...
        with engine.connect() as conn:
            ...
            total_companies = conn.execute(text(f"SELECT COUNT(*) FROM {dbt_table('dim_companies')}")).scalar()
            duplicate_rate = duplicate_companies / total_companies * 100 if total_companies > 0 else 0
            
            assert duplicate_rate <= 10.0, f"Demasiadas empresas duplicadas: {duplicate_rate:.2f}%"
            print(f"✅ Calidad normalización empresas: {duplicate_rate:.2f}% duplicados")
            
            if duplicate_rate > 5.0:
                print(f"⚠️  Tasa de duplicación alta ({duplicate_rate:.2f}%) - considerar mejorar normalización")
    ...
    def test_geographic_data_consistency(self):
        ...
        with engine.connect() as conn:
            ...
            assert orphaned_locations == 0, f"Ubicaciones huérfanas: {orphaned_locations}"
            ...
            assert countries_with_locations >= 10, f"Muy pocos países con ubicaciones: {countries_with_locations}"
            print(f"✅ Consistencia geográfica: {countries_with_locations} países con ubicaciones")
    ...
    def test_temporal_data_validity(self):
        ...
        with engine.connect() as conn:
            ...
            assert date_coverage >= 50.0, f"Cobertura de fechas muy baja: {date_coverage:.2f}%"
            
            print(f"📊 Cobertura temporal: {date_coverage:.2f}% ({jobs_with_dates:,}/{total_jobs:,})")
            ...
            if min_date:
                ...
                if min_date_check < cutoff_date:
                    print(f"⚠️  DATOS HISTÓRICOS: Fecha mínima {min_date} (más de 1 año)")
                    print("   Esto puede ser normal para datasets de análisis histórico")
                else:
    ...
    def test_business_rules_enforcement(self):
        ...
        with engine.connect() as conn:
            ...
            total_skills = conn.execute(text(f"SELECT COUNT(*) FROM {dbt_table('dim_skills')}")).scalar()
            malformed_rate = malformed_skills / total_skills * 100 if total_skills > 0 else 0
            
            assert malformed_rate <= 5.0, f"Demasiados skills mal formateados: {malformed_rate:.2f}%"
            
            print("✅ Reglas de negocio validadas")
            if malformed_skills > 0:
                print(f"⚠️  {malformed_skills} skills con formato cuestionable ({malformed_rate:.2f}%)")
